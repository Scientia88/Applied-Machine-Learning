...
#     df['rolling_mean_5'] = df['close'].rolling(window=5).mean()
#     df['rolling_mean_7'] = df['close'].rolling(window=7).mean()
#     last_record = df.iloc[[-1]].reset_index(drop=True)  
...
#     # Optimize hyperparameters with Optuna for 10 base models
#     params = []
#     for i in range(1,11):
#         study = optuna.create_study(direction='minimize')
#         study.optimize(objective, n_trials=100)
...
#     # Define a function to train each XGBoost model
#     def train_xgboost_model(X_train, y_train, param):
#         model = xgb.XGBRegressor(**param)
#         model.fit(X_train, y_train)
#         return model
...
#     perform = pd.DataFrame(columns=['current', 'future', 'predicted', 'return', 'investment', 'diff','resulte', 'actual dir', 'predicted dir'], 
#                                index=range(len(df) - j))
    
#     for i in range(len(df) - j):
#         # Creating train and test datasets for base models
#         dff = df.iloc[i:j, :]
#         split_index = int(len(dff) * 0.8)
#         X_train = dff.iloc[:split_index][df.columns[0:-1]]
#         Y_train = dff.iloc[:split_index][df.columns[-1]]
#         X_test = dff.iloc[split_index:][df.columns[0:-1]]
#         Y_test = dff.iloc[split_index:][df.columns[-1]]
        
        
#         # Train the models in parallel
#         models = Parallel(n_jobs=-1)(delayed(train_xgboost_model)(X_train, Y_train, param)
#                                       for param in params)
        
#         # Vectorize the predictions from the base models
#         base_model_preds_train = [model.predict(X_train) for model in models]  # List of predictions from each model
#         # Convert predictions to a DataFrame (or matrix) to use as price estimator
#         train_meta_features = np.column_stack(base_model_preds_train)
        
#         base_model_preds_test = [model.predict(X_test) for model in models]
#         test_meta_features = np.column_stack(base_model_preds_test)
        
#         # Use actual prices for training target in regression
#         train_meta_target = Y_train
        
#         # Use actual prices for test target in regression
#         test_meta_target = Y_test
    
    
#         meta_model = RandomForestRegressor(n_estimators=100,max_depth=3)
#         meta_model.fit(train_meta_features, train_meta_target)
    
#         # Make final predictions using the meta-model
#         train_final_predictions = meta_model.predict(train_meta_features)
#         test_final_predictions = meta_model.predict(test_meta_features)
        
        
#         predictions = pd.DataFrame(test_final_predictions,index = test_meta_target.index,columns=['predicted'])
#         Y_test = pd.DataFrame(Y_test).join(predictions)
...
#         if ((perform.loc[exp,'future'] - perform.loc[exp,'current']) * (perform.loc[exp,'predicted'] - perform.loc[exp,'current'])) > 0:    
#             perform.loc[exp, 'return'] = abs((perform.loc[exp,'future'] - perform.loc[exp,'current']) / perform.loc[exp,'current'])
#             invest = invest * (1 + perform.loc[exp,'return'])
#             perform.loc[exp, 'investment'] = invest
#             perform.loc[exp, 'resulte'] = 1
#             if (perform.loc[exp,'future'] - perform.loc[exp,'current']) > 0:
#                 perform.loc[exp,'actual dir'] = 1
#             else:
#                 perform.loc[exp,'actual dir'] = 0
#             if  (perform.loc[exp,'predicted'] - perform.loc[exp,'current']) > 0:
#                 perform.loc[exp,'predicted dir'] = 1
#             else:
#                 perform.loc[exp,'predicted dir'] = 0
                                    
                
#         else:
#             perform.loc[exp, 'return'] = -abs((perform['future'][exp] - perform['current'][exp]) / perform['current'][exp])
#             invest = invest * (1 + perform.loc[exp,'return'])
#             perform.loc[exp, 'investment'] = invest
#             perform.loc[exp, 'resulte'] = 0
#             if (perform.loc[exp,'future'] - perform.loc[exp,'current']) > 0:
...
#         perform.loc[exp, 'diff'] = perform['future'][exp] - perform['predicted'][exp]
#         exp += 1
#         j += 1
            
#      # Convert perform dataframe from object to float and drop missing values
#     perform = perform.apply(pd.to_numeric, errors='coerce')
#     perform.dropna(inplace=True)    
...
#     prediction = meta_model.predict(np.column_stack([model.predict(last_record) for model in models]))
#     current = last_record.close.values[0]
#     pc = (prediction-current)*100/current
#     pc = round(pc[0],3)
#     print('********************************************************************************************')
#     if pc > 0:
#         print(f'The current price is {current} and the Predicted price is {prediction[0]}')
#         print(f'There is a buy opportunity with expected price change of {pc}%')
#     elif pc < 0:
#         print(f'The current price is {current} and the Predicted price is {prediction[0]}')
#         print(f'There is a sell opportunity with expected price change of {pc}%')
#     else:
#         print('No expected volatility for today, enjoy your coffee!')
#     print('********************************************************************************************')
...
#     # Daily Return and Statistics
#     dr = perform['return'].mean()
#     maxreturn = perform['return'].max()
#     maxloss = perform['return'].min()
    
#     # Monthly Return and Sharpe Ratio
#     mr = (1 + dr) ** 21 - 1  # Assuming 21 trading days in a month
#     dsr = perform['return'].mean() / perform['return'].std()  # Daily Sharpe Ratio
#     msr = dsr * np.sqrt(21)  # Monthly Sharpe Ratio
...
#     # 1. Plot: Last year Actual and Predicted Prices Over Time in the D1 Timeframe
#     plt.figure(figsize=(12, 6))
...
#     plt.xlabel('Time')
#     plt.ylabel('Close Price')
#     plt.title('Last Year Actual and Predicted Prices Over Time (D1 Timeframe)')
#     plt.legend()
#     plt.show()

#     # 2. Plot: Capital Growth in the Last Year
#     plt.figure(figsize=(12, 6))
#     plt.plot(perform.index[-252:-1], perform['investment'].iloc[-252:-1], label='Accumulated Investment')
#     plt.xlabel('Time')
#     plt.ylabel('Investment Value')
#     plt.title('Capital Growth in the Last Year')
#     plt.legend()
...
#     # 3. Plot: Return Distribution with Mean and Quantiles
#     mean = perform['return'].mean()
#     q05 = perform['return'].quantile(0.05)
#     q95 = perform['return'].quantile(0.95)
...
#     # 4. Plot: Q-Q Plot of Residuals
#     plt.figure(figsize=(10, 6))
#     stats.probplot(perform['diff'], dist="norm", plot=plt)
#     plt.title('Q-Q Plot of Residuals')
#     plt.show()

#     # 5. Plot: Autocorrelation of Residuals
#     plt.figure(figsize=(10, 6))
#     plot_acf(perform['diff'], lags=30)
#     plt.title('Autocorrelation of Residuals')
#     plt.show()
