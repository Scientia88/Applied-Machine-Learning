...
def wf_training(df):
    """
    Train an ensemble model with XGBoost as base models and Random Forest as the meta-model using walk-forward validation.
    
    Args:
        df (pd.DataFrame): DataFrame with historical stock prices, where 'close' column contains closing prices.
        
    Returns:
        perform (pd.DataFrame): DataFrame containing investment performance metrics and predictions.
    """
    
    # Feature Engineering - Generate date-based and lag features
    ...
    # Keep the last record for final prediction and set the target as next day's close price
    last_record = df.iloc[[-1]].reset_index(drop=True)
    df['target'] = df['close'].shift(-1)
    df.dropna(inplace=True)
    
    # Initial dataset for model training and validation
    dfp = df.head(1000)
    ...
    # Define a function to train each XGBoost model with specified parameters
    def train_xgboost_model(X_train, y_train, param):
        model = xgb.XGBRegressor(**param)
        model.fit(X_train, y_train)
        return model

    # Initialize variables for walk-forward validation
    j, step, exp, invest = 1000, 1, 0, 1000
    perform = pd.DataFrame(columns=['current', 'future', 'predicted', 'return', 'investment', 
                                    'diff', 'resulte', 'actual dir', 'predicted dir'], 
                           index=range(len(df) - j))

    # Walk-forward validation loop
    for i in range(0, len(df) - j, step):
        # Expanding training and test datasets
        dff = df.iloc[:j + i, :]
...
def wf_analyse_performance(perform):
    """
    Analyze and visualize the performance of the investment strategy based on the ensemble model predictions.
    ...
    # Plot: Last Year’s Actual and Predicted Prices in D1 Timeframe
